...
- **Documentation:** [linkedrecords.com](https://linkedrecords.com) · [Getting Started Guide](https://linkedrecords.com/getting-started)
...
The server is then available at `http://localhost:6543`. Connect to it from your app:

```bash
npm install @linkedrecords/browser
```

```js
import LinkedRecords from '@linkedrecords/browser';

const lr = new LinkedRecords(new URL('http://localhost:6543'), {
  client_id: 'linkedrecords-dev',
...
// To start the login flow (e.g., on a button click):
// lr.login();

// Create a record only you can access - share it later by creating facts
const todo = await lr.Record.createKeyValue(
  { title: 'Buy groceries', done: false },
  [['$it', 'isA', 'Todo']],
);
```

Using React? The [Getting Started Guide](https://linkedrecords.com/getting-started) builds a complete collaborative todo app with `@linkedrecords/react` hooks.
...
- Flexibility: The few available endpoints can be composed to support a variety of use cases. The baked in authorization model should allow to
...
LinkedRecords is configured via environment variables. See tables below, or the
[full configuration reference](https://linkedrecords.com/configuration) with
...
| QUOTA_COUNT_KV_RECORDS | false | If the storage space for KeyValue records is deducted from the accountee quota. |
| QUOTA_COUNT_LT_RECORDS | false | If the storage space for LongText records is deducted from the accountee quota. |
| ENABLE_AUTH_RULE_CACHE | false | Enable cache for authorization lookups. Might require a lot of memory. |
| SHORT_LIVED_ACCESS_TOKEN_SIGNING | xxxx | Configuring this is optional but can reduce load on the database because short lived access tokens will be used for checking access when a client subscribes to record changes. |
...
| AUTH_IDP_LOGOUT | true | When set to true the user session will be destroyed in the application AND within the identity provider. |
...
If S3 is configured it will be used to store blob record values. If it is not configured they will be stored in the PostgreSQL database.
It is recommended to configure S3.

| Environment Variable Name | Example | Description |
| ------------------------- | ------- | ----------- |
| S3_COPY_FROM_BL_ATTRIBUTE_TABLE | false | This is used for migrating blob storage from postgresql to S3. |
...
Redis is only needed when running multiple LinkedRecords instances behind a load balancer, so real-time updates reach clients connected to other instances.

| Environment Variable Name | Example | Description |
| ------------------------- | ------- | ----------- |
| REDIS_HOST | redis.example.com | The Redis host. Setting this enables the Redis adapter. |
| REDIS_USERNAME | default | The Redis username. |
| REDIS_PASSWORD | xxx | The Redis password. |
...
Prerequisites: Node.js v24 and a PostgreSQL database (configured via the `PG*` environment variables, see [CONTRIBUTING.md](CONTRIBUTING.md); alternatively set `USE_PGLITE=true` to use an embedded database).

```bash
npm install
npm start              # Build and run the server on port 6543
npm test               # Run unit/component tests
npm run wdio           # Run end-to-end tests
npm run lint           # ESLint
```

The documentation website lives in `docs/`:
...
The load test simulates a realistic document management scenario. Each **createDocument** operation creates a blueprint of 8 records (7 KeyValueRecords + 1 LongTextRecord) including the document content, collaborator/reader groups, comments, references, and configuration. The **fetchDocuments** operation lists all documents for a user, while **fetchDocument** retrieves a single document with all its associated records.
...
|-----------|-------------|
| `createDocument` | Time to create a new document with all related records (content, config, comments, references, collaborator/reader groups). You can see that `createDocument` is independent of the total amount of documents in the database as well as the amount of documents visible to the user.|
