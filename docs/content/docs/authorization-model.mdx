...
const doc = await lr.Record.createKeyValue({ title: 'My Notes' });
...
Indicates ownership and accountability for a record. The accountable party
is responsible for the storage quota used by the record.

```typescript
[userId, '$isAccountableFor', recordId]
```

<Callout type="info">
When you create a record, a `$isAccountableFor` fact linking you to it is
...
<Callout type="info">
**What is a group?** There is no special "group" concept in LinkedRecords. A group is simply a record (typically a KeyValue record) that you use as a node for memberships. You create a record, and then use its ID as the target of `$isMemberOf` facts. You don't need to tag it with `isA` or assign any terms - any record can serve as a group.

```typescript
// Create a simple record - this becomes your "group"
const team = await lr.Record.createKeyValue({ name: 'Engineering' });
...
// Grant the group access to another record
await lr.Fact.createAll([
...
Members can:
- Read and write records the group has `$canAccess` to
- Read records the group has `$canRead` to
...
Grants read-only access to a record.

```typescript
[userId, '$canRead', recordId]
// or for a group:
[teamId, '$canRead', recordId]
```

### $canAccess

Grants read and write access to a record.

```typescript
[userId, '$canAccess', recordId]
// or for a group:
[teamId, '$canAccess', recordId]
```

### $canRefine

Grants permission to use a record as the **subject** in facts (conceptor permission).

```typescript
[teamId, '$canRefine', recordId]
```

This allows team members to create facts like `[recordId, 'somePredicate', someOtherRecordId]`.

### $canReferTo

Grants permission to use a record as the **object** in facts (referrer permission).

```typescript
[teamId, '$canReferTo', recordId]
```

This allows team members to create facts like `[otherId, 'somePredicate', recordId]`.

## Permission Hierarchy

### Creator Permissions

When you create a record, you automatically get full permissions:
...
```typescript
// Create a team. A team is just a record.
const team = await lr.Record.createKeyValue({ name: 'Engineering' });

// Create a document with team access
const doc = await lr.Record.createKeyValue({ title: 'Spec' }, [
  ['$it', 'isA', 'Document'],
...
const myNotes = await lr.Record.createKeyValue({
...
const team = await lr.Record.createKeyValue({ name: 'Marketing' }, [
  ['$it', 'isA', 'Team'],
]);
...
const org = await lr.Record.createKeyValue({ name: 'Acme' }, [
  ['$it', 'isA', 'Organization'],
]);
...
When a group has `$canAccess` to a record, members of that group inherit implicit permissions:

- **Read and write access** to the record's payload
- **Implicit `$canRefine`** - can use the record as subject in custom facts (if group has `$canRefine`)
- **Implicit `$canReferTo`** - can use the record as object in custom facts (if group has `$canReferTo`)
...
This means that for creating custom predicates, users with `$canAccess` through group membership effectively have `$canRefine` and `$canReferTo` permissions on those records.
