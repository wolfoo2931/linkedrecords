...
|------------|-----------|--------------|
| Read-Only | `$canRead` | View record values |
| Read-Write | `$canAccess` | View and modify record values |
...
### Share an Existing Record with a Team
...
Beyond reading and writing record values, you can share the ability to
create facts about a record:

### Allow Creating Facts with Record as Subject

```typescript
// Allow team members to add classifications to this record
await lr.Fact.createAll([
...
### Allow Creating Facts with Record as Object

```typescript
// Allow team members to reference this record in facts
await lr.Fact.createAll([
...
const { myDocs } = await lr.Record.findAll({
  myDocs: [
    ['$it', 'isA', 'Document'],
  ],
});
