...
const todos = useKeyValueRecords([['$it', 'isA', 'Todo']]);
```
...
With LinkedRecords, your entire application is frontend code. No backend routes, no database queries, no sync logic, no state management boilerplate. You create facts, create records, query records - everything is reactive and declarative.
...
The `useKeyValueRecords` hook (from [`@linkedrecords/react`](/react)) handles everything:
...
// Create a new todo - automatically appears in all useKeyValueRecords queries
...
const todoAttr = await lr.Record.find(todoId);
...
const myDoc = await client.Record.createKeyValue(
...
const projects = useKeyValueRecords([['$it', 'isA', 'Project']]);
...
Since data follows a consistent pattern (facts and records), you can build components that work in any application:

```tsx
// A profile component that works in ANY app connected to your LinkedRecords
function UserProfile({ userId }) {
  const [profile] = useKeyValueRecords([
    ['$it', 'isA', 'UserProfile'],
