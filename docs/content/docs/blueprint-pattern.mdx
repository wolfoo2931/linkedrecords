...
description: Creating multiple related records atomically
---
...
const org = await lr.Record.createKeyValue({ name: 'Acme Inc' });
const team = await lr.Record.createKeyValue({ name: 'Admin Team' });
...
const { org, team } = await lr.Record.createAll({
  org: {
...
```typescript
const result = await lr.Record.createAll({
  recordName: {
...
`LongTextAttribute`, `BlobAttribute`) - "Attribute" is the former name of the
Record concept, and the runtime identifiers were kept for backwards
compatibility. It defaults to `KeyValueAttribute` when omitted.
</Callout>
...
```typescript
const { parent, child } = await lr.Record.createAll({
  parent: {
    type: 'KeyValueAttribute',
    value: { name: 'Parent' },
    facts: [['$it', 'isA', 'Parent']],
...
const { list } = await lr.Record.createAll({
  list: {
    type: 'KeyValueAttribute',
...
```typescript
const { doc, notes, attachment } = await lr.Record.createAll({
  doc: {
    type: 'KeyValueAttribute',
...
2. **Use meaningful names** - Choose descriptive record names in the blueprint
   for clarity
