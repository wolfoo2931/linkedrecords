...
**Example:**
```bash
curl "https://api.example.com/records?query=%7B%22docs%22%3A%5B%5B%22%24it%22%2C%22isA%22%2C%22Document%22%5D%5D%7D"
```
...
```
GET /records/:nodeId/members
```

**Parameters:**
- `nodeId` - Team/group record ID
- `clientId` - Client identifier (query parameter)
...
```
WebSocket: /records/:id/changes?clientId=xxx
```
