...
Accountability in LinkedRecords determines who "owns" a record in terms of
...
```typescript
const doc = await lr.Record.createKeyValue({ title: 'My Doc' });
...
async function createOrganization(lr, name: string) {
  const { org, team, resourceCollection } = await lr.Record.createAll({
    org: {
...
async function createOrgDocument(lr, org: { id: string, team: { id: string } }, title: string) {
  return lr.Record.createKeyValue(
...
const { myResources } = await lr.Record.findAll({
  myResources: [
...
const workDoc = await lr.Record.createKeyValue(
