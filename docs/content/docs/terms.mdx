...
Once declared, you can use terms to classify your records:
...
// Now you can use it
const invoice = await lr.Record.createKeyValue(
...
await userB.Record.createKeyValue(
  { title: 'My First Post' },
  [['$it', 'isA', 'BlogPost']]
);
```
...
Find records of multiple types:

```typescript
const { documents, projects, tasks } = await lr.Record.findAll({
...
```typescript
// WRONG: Using a term without declaring it first
...
```typescript
// User A creates a private document record
await userA.Record.createKeyValue(
  { secret: 'data' },
  [['$it', 'isA', 'Document']]
);

// User B can query for Documents, but won't see User A's private record
