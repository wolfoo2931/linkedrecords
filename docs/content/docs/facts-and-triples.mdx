...
When creating records with associated facts, use `$it` as a placeholder that
refers to the record being created:
...
The `$it` placeholder is replaced with the actual record ID after creation.

## Creating Facts

### With Record Creation

The most common way to create facts is when creating a record:
...
### Finding Records by Facts

Use `Record.findAll()` to query records based on their associated facts:
...
const { projectDocs } = await lr.Record.findAll({
  projectDocs: [
    ['$it', 'isA', 'Document'],
...
authorization semantics. See the [Authorization Model](/authorization-model)
...
2. **Declare terms first** - Before using a term as an object in `isA` facts,
   declare it using `$isATermFor`. See [Terms](/terms) for details.
...
4. **Use system predicates correctly** - Reserved predicates (starting with `$`)
   have specific authorization implications. Review the [Authorization Model](/authorization-model)
   before using them.

## Related

- [Query Language](/query-language) - Complete reference for querying facts and records
- [Terms](/terms) - Declaring terms before using them in facts
