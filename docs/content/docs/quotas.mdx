...
or organization reaches their quota limit, further record creation or modification
...
quota is charged for storage used by a record.
...
1. **Record creation is blocked** - New records cannot be created
2. **Record modification is blocked** - Existing records cannot be updated
...
try {
  await lr.Record.createKeyValue({ /* large data */ });
} catch (error) {
...
const personalDoc = await lr.Record.createKeyValue(
  { title: 'My Notes' },
  [['$it', 'isA', 'Document']]
);

// Organization document (counts against org's quota)
const orgDoc = await lr.Record.createKeyValue(
  { title: 'Project Spec' },
  [
    ['$it', 'isA', 'Document'],
...
| `QUOTA_COUNT_KV_ATTRIBUTES` | Count KeyValue records in quota | false |
| `QUOTA_COUNT_LT_ATTRIBUTES` | Count LongText records in quota | false |

<Callout type="info">
Blob records are always counted in quota calculations when S3 storage is
configured.
</Callout>
