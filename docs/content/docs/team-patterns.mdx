...
A team is simply a record with the appropriate permission facts:
...
// Create the team
const team = await lr.Record.createKeyValue(
...
const myId = await nemo.ensureUserIdIsKnown();
...
```typescript
const myId = await member.ensureUserIdIsKnown();
...
```typescript
const { org, adminTeam, editorTeam, viewerTeam } = await lr.Record.createAll({
  org: {
...
const document = await lr.Record.createKeyValue(
