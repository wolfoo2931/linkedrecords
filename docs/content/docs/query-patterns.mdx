...
```typescript
const result = await lr.Record.findAll({
  tasks: [
    ['$it', 'isA', 'Task'],
  ],
  projects: [
...
// result.tasks is an array of Task records
// result.projects is an array of Project records
```
...
Filter by record type (KeyValueAttribute, LongTextAttribute, or BlobAttribute):

```typescript
const { documents } = await lr.Record.findAll({
  documents: [
...
```typescript
const { activeTasks } = await lr.Record.findAll({
  activeTasks: [
    ['$it', 'isA', 'Task'],
...
rather than deleting old ones. It returns records where the most recent fact
with that predicate matches the specified value.
</Callout>

### $not(value)

Negation operator - find records where a predicate does NOT have a specific value:

```typescript
const { incompleteTasks } = await lr.Record.findAll({
  incompleteTasks: [
    ['$it', 'isA', 'Task'],
...
```typescript
const { activeItems } = await lr.Record.findAll({
  activeItems: [
    ['$it', 'isA', 'TodoList'],
...
Query specific records by their ID:

```typescript
// Single record
const { doc } = await lr.Record.findAll({
  doc: documentId,  // Just pass the ID as a string
});

// Multiple specific records
const { doc1, doc2 } = await lr.Record.findAll({
  doc1: 'kv-abc123',
  doc2: 'kv-def456',
});
```
...
```typescript
const { results } = await lr.Record.findAll({
  results: [
    ['$it', '$hasDataType', 'KeyValueAttribute'],
    ['$it', 'isA', 'TodoList'],
...
```typescript
const { listItems } = await lr.Record.findAll({
  listItems: [
...
```typescript
const { authoredDocuments } = await lr.Record.findAll({
  authoredDocuments: [
    ['$it', 'isA', 'Document'],
...
const { projectTasks } = await lr.Record.findAll({
  projectTasks: [
    ['$it', 'isA', 'Task'],
    ['$it', 'belongsTo', projectId],
  ],
...
```typescript
const { orgResources } = await lr.Record.findAll({
  orgResources: [
...
A common pattern is using separate state records to track item status.

### Why Records Instead of Terms?

You might wonder: why create records for states instead of using terms directly
(e.g., `[taskId, 'stateIs', 'Completed']`)? The answer is **permission control**.

When states are records, you can use `$canReferTo` to control which users or
...
// 2. Create state records
const { activeState, archivedState, completedState } = await lr.Record.createAll({
  activeState: {
...
// 5. Query by state
const { completedTasks } = await lr.Record.findAll({
  completedTasks: [
    ['$it', 'isA', 'Task'],
...
const { pendingTasks } = await lr.Record.findAll({
  pendingTasks: [
    ['$it', 'isA', 'Task'],
...
const { active } = await lr.Record.findAll({
  active: [
    ['$it', 'isA', 'Task'],
...
Queries automatically respect authorization - you only see records you have
access to:

```typescript
// User A creates a private document
await userA.Record.createKeyValue(
  { title: 'Secret' },
  [['$it', 'isA', 'Document']]
);

// User B queries for documents
const { docs } = await userB.Record.findAll({
  docs: [['$it', 'isA', 'Document']],
});
// docs is empty - User B can't see User A's private document
```

## Loading Record Values

After querying, you can get record values:
...
Or use `findAndLoadAll()` for batch loading records:

```typescript
const { tasks } = await lr.Record.findAndLoadAll({
  tasks: [['$it', 'isA', 'Task']],
});
...
4. **Avoid fetching everything** - Query for what you need rather than loading
   all records and filtering client-side

## Best Practices

1. **Use $latest for state tracking** instead of deleting old state facts

2. **Create dedicated state records** for reusable state markers
