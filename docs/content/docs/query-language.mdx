...
LinkedRecords provides a declarative query language for finding records and
facts. Queries are expressed as patterns that match against the triplestore.

There are two main query methods:
- `Record.findAll()` - Find records matching fact patterns
...
For example:
- `['$it', 'isA', 'Document']` - Match records that are Documents
- `[userId, '$isMemberOf', '$it']` - Match records the user is a member of
- `['$it', 'belongsTo', projectId]` - Match records belonging to a project
...
// result.queryName is an array of matching records
```

Each named query group returns an array of records that match **all** patterns
...
```typescript
const { tasks, projects, users } = await lr.Record.findAll({
  tasks: [
    ['$it', 'isA', 'Task'],
  ],
  projects: [
...
```typescript
const { doc, author } = await lr.Record.findAll({
  doc: 'kv-abc123',           // Direct ID lookup
  author: [                    // Pattern-based query
    ['$it', 'isA', 'Author'],
  ],
});
// doc is a single record (or undefined if not found/accessible)
// author is an array of records
```
...
```typescript
// Find all facts about a specific record
const facts = await lr.Fact.findAll({
  subject: [attributeId],
});

// Find facts about records matching a pattern
const facts = await lr.Fact.findAll({
...
```typescript
// Find all facts pointing to a specific record
const facts = await lr.Fact.findAll({
  object: [attributeId],
});
...
You only see records and facts you have access to:

```typescript
// Alice creates a private document
await alice.Record.createKeyValue(
  { title: 'Private' },
  [['$it', 'isA', 'Document']]
);

// Bob queries for documents
const { docs } = await bob.Record.findAll({
  docs: [['$it', 'isA', 'Document']],
});
...
// Record values already loaded - no additional network requests
for (const task of tasks) {
...
```typescript
await lr.Record.subscribeToQuery(
  {
    tasks: [
      ['$it', 'isA', 'Task'],
      ['$it', 'belongsTo', projectId],
...
const { completedState, archivedState } = await lr.Record.createAll({
  completedState: {
...
const project = await lr.Record.createKeyValue(
  { name: 'Website Redesign' },
  [['$it', 'isA', 'Project']]
);

const task1 = await lr.Record.createKeyValue(
...
- [Query Patterns](/query-patterns) - Advanced patterns and use cases for records
