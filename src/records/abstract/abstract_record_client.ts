...
export default abstract class AbstractAttributeClient <Type, TypedChange extends IsSerializable > {
  ...
  public async create(value: Type, facts?: [ string?, string?, string? ][]) {
    ...
    const url = `/records?dtp=${this.getDataTypePrefix()}&clientId=${this.clientId}`;
  ...
  public getDataURL() {
    const url = new URL(`/records/${this.id}?clientId=${this.clientId}&valueOnly=true`, this.linkedRecords.serverURL);
    return url.toString();
  }
  ...
  public async load(serverState?: {
    ...
    if (!result) {
      const url = `/records/${this.id}?clientId=${this.clientId}`;
    ...
    if (!this.attrSubscription) {
      const url = new URL(`/records/${this.id}/changes?clientId=${this.clientId}`, this.serverURL);
