...
export default class BlobRecord extends AbstractAttributeClient<Blob, BlobChange> {
  ...
  public getDataTypePrefix() : string {
    return BlobRecord.getDataTypePrefix();
  }

  public getDataTypeName() : string {
    return BlobRecord.getDataTypeName();
  }
  ...
  protected async transmitChange(value: Blob): Promise<boolean> {
    ...
    const formData = new FormData();
    const url = `/records/${this.id}?clientId=${this.clientId}&actorId=${this.actorId}`;
...
export { BlobRecord as BlobAttribute };
