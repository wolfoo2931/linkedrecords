...
export default class LongTextRecord extends AbstractAttributeClient<string, LongTextChange> {
  ...
  public getDataTypeName() : string {
    return LongTextRecord.getDataTypeName();
  }
  ...
  protected async rawChange(changeset: LongTextChange): Promise<boolean> {
    ...
    if (this.changeInTransmission) {
      if (
        this.getLastChangeTransmittedMillisecondsAgo() > LongTextRecord.serverApprovalTimeoutInMS
      ) {
        this.linkedRecords.handleConnectionError(new Error(`No approval received from server after ${LongTextRecord.serverApprovalTimeoutInMS} ms`));
      }
...
export { LongTextRecord as LongTextAttribute };
